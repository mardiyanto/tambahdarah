...
$pdf->Cell(0,5,$k_k[nama],'0','1','C',false);
$pdf->SetFont('Arial','i',8);
$pdf->Cell(0,5,$k_k[alamat],'0','1','C',false);
$pdf->Cell(0,2,$k_k[telp],'0','1','C',false);
$pdf->Ln(3);
...
if(@$_GET['data'] == "pengajar") {
	...
	$pdf->SetFont('Arial','B',11);
	$pdf->Cell(50,5,'Laporan Data Nilai mahasiswa','0','1','L',false);
	$pdf->Ln(5);
	$pdf->SetFont('Arial','i',10);
	$pdf->Cell(25,6,'Nama mahasiswa',0,0,'L',false);
...
else if(@$_GET['data'] == "nilaipersiswaall") {
	$id_tq = @$_GET['id_tq'];
	$sql_tq = mysqli_query($db, "SELECT * FROM tb_topik_quiz JOIN tb_mapel ON tb_topik_quiz.id_mapel = tb_mapel.id WHERE id_tq = '$id_tq'") or die($db->error);
	$data_siswa_login = mysqli_fetch_array($sql_tq);
	$pdf->SetFont('Arial','B',11);
	$pdf->Cell(50,5,'Laporan Data Nilai mahasiswa','0','1','L',false);
	$pdf->Ln(5);
	$pdf->SetFont('Arial','i',10);
	...
	while($data_siswa_mengikuti_tes = mysqli_fetch_array($sql_siswa_mengikuti_tes)) {
		...
		$sql_cek_jawaban = mysqli_query($db, "SELECT * FROM tb_jawaban WHERE id_tq = '$data_nilai_pilgan[id_tq]' ") or die ($db->error);
		$data_jawaban = mysqli_fetch_array($sql_cek_jawaban);
		if(mysqli_num_rows($sql_cek_jawaban) > 0) {
			$sql_cek_nilai_essay = mysqli_query($db, "SELECT * FROM tb_nilai_essay WHERE id_tq = '$data_nilai_pilgan[id_tq]'") or die ($db->error);
			$data_nilai_essay = mysqli_fetch_array($sql_cek_nilai_essay);
			if(mysqli_num_rows($sql_cek_nilai_essay) > 0) {
				$pdf->Cell(65,6,$data_nilai_essay['nilai'],1,0,'C');
				$tot = 
...
else if(@$_GET['data'] == "quizpilganda") {
	$id_tq = @$_GET['id_tq'];
	$sql_tq = mysqli_query($db, "SELECT * FROM tb_topik_quiz JOIN tb_mapel ON tb_topik_quiz.id_mapel = tb_mapel.id WHERE id_tq = '$id_tq'") or die($db->error);
	$data_tq = mysqli_fetch_array($sql_tq);
	$pdf->SetFont('Arial','B',11);
